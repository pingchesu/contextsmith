...
export function describeScope(resources: Resource[], selectedIds: string[]) {
  ...
  const names = selectedIds.map((id) => resources.find((resource) => resource.id === id)?.name ?? short(id));
  return names.join(', ');
}
