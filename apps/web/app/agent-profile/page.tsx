...
const REVIEW_PROMPTS = [
  ...
  { id: 'runtime', label: 'Runtime behavior', query: 'Describe the runtime behavior and operating constraints this agent should follow. Include production-action boundaries, MCP/context endpoints, and evidence discipline. Cite source context.' },
...
export default function AgentProfilePage() {
  ...
  return <main className="page">
    <PageHeader eyebrow="Agent Profile" title="Generated repo agent content" description="This page is the review surface for the generated agent itself: identity, runtime instructions, resource coverage, generated context packets, citations, and symbols. No server/vim review required." />
    {!agent ? <EmptyState text="Agent profile is loading or unavailable." /> : <>
      ...
      <Card><h2>Agent manifest</h2><p className="muted">Human-readable manifest of what was generated. IDs are debug metadata only; reviewers should reason from project/resource names and evidence below.</p><div className="grid two"><div><div className="label">Agent</div><strong>{agent.name}</strong><p className="muted">{agent.description || 'Generated knowledge agent for this project.'}</p></div><div><div className="label">Workspace / project</div><strong>{workspace?.name ?? 'Workspace'} / {project?.name ?? 'Project'}</strong><div className="code">{short(agent.workspace_id)} / {short(agent.project_id)}</div></div><div><div className="label">Agent context endpoint</div><div className="code">{agent.agent_context_endpoint}</div></div><div><div className="label">MCP endpoint</div><div className="code">{agent.mcp_endpoint}</div></div></div></Card>
      <div className="grid two"><Card><h2>Runtime policy</h2><p className="muted">This policy is attached to the agent and should be reviewed before exposing it to a runtime.</p><pre className="code-block light">{JSON.stringify(agent.tool_policy, null, 2)}</pre></Card><Card><h2>Provider and freshness</h2><div className="grid"><StatusChip value={provider?.status ?? 'unknown'} /><div className="code">{provider ? JSON.stringify(provider.embedding, null, 2) : 'Provider not loaded'}</div><p className="muted">Last indexed: {fmt(agent.last_index_finished_at)}</p></div></Card></div>
      <Card><h2>Resource contribution map</h2><p className="muted">This tells you what the repo agent is made of. Use this before approving the agent: resource status, review status, freshness, usage, current snapshot, and generated graph coverage.</p><div className="table-wrap"><table><thead><tr><th>Resource</th><th>Review/freshness</th><th>Usage</th><th>Snapshot</th><th>Refresh</th></tr></thead><tbody>{resources.map((resource) => { const review = reviewByResource.get(resource.id); const usage = usageByResource.get(resource.id); return <tr key={resource.id}><td><strong>{resource.name}</strong><div className="code">{resource.type} · {resource.uri}</div></td><td><StatusChip value={resource.review_status} /> <StatusChip value={review?.freshness_status ?? 'unknown'} /><div className="code">{review?.stale_reasons.join(', ') || 'no stale reasons'}</div></td><td>{usage?.hit_count ?? 0} hits<div className="code">last {fmt(usage?.last_used_at)}</div></td><td className="code">{short(resource.current_snapshot_id)}</td><td>{resource.update_frequency}<div className="code">{fmt(resource.last_refresh_finished_at)}</div></td></tr>; })}</tbody></table></div></Card>
      ...
      <Card><h2>System prompt override</h2>{agent.system_prompt ? <pre className="code-block light">{agent.system_prompt}</pre> : <EmptyState text="No custom system prompt configured. Runtime receives ContextSmith default read-only/citation discipline." />}</Card>
    </>}
  </main>;
}
