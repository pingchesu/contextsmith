...
| Agent Pack install model | SourceBrief's default Agent Pack / Skill Export mode is `remote-live`: installed packs are thin adapters that route agents back to SourceBrief for current cited evidence, not full local corpus sync. | Current | `README.md`, `docs/AGENT_PACKS.md`, `docs/CONCEPTS.md`, `docs/STATUS.md`, `tests/unit/test_cli.py` `agent_pack_doctor` / `agent_pack_docs` selections. | `pinned-snapshot` and `local-mirror` are explicit non-default manifest policy validations. Do not claim SourceBrief ships `agent-pack export --mode local-mirror` or local-mirror install/update/purge lifecycle. |
