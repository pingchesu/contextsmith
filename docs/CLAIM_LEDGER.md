...
| Screenshot-backed 50Q walkthrough | Current 50Q walkthrough shows the local-alpha proof path with session login, 50/50 mechanical pass, and clean browser console/network transcript. | RISK | `docs/evaluations/sourcebrief-launch-50q-20260630.md`, `docs/assets/screenshots/launch-50q-20260630/`. | Current proof is useful but #234 tracks runner fail-fast/token cleanup, screenshot UUID hygiene, and isolated-stack reproducibility gaps before this becomes a fully automated/public proof gate. |
...
| Security boundaries | Workspace/project/resource/token boundaries have targeted tests and live probe coverage, but the launch-security probe itself has known gate-quality gaps. | RISK | `scripts/launch_security_probe.py`, `make launch-security-probe`, `tests/unit/test_launch_security_probe.py`, auth/security integration tests, `qa_smoke.py` denial checks. | #233 tracks false-pass, JSON redaction, browser-transcript, token-lifecycle, dev-auth, and cleanup issues before security boundaries can be called a launch PASS. |
